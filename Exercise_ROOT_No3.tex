...
\noindent In this third ROOT exercise we cover the following new and frequently used class in ROOT:
%
\begin{enumerate}
...
%
\noindent c) In case you downloaded from the HEPData website ALICE measurements in the ROOT file, note the ROOT command-line utility {\rm\bf rootls}, which you can use (if ROOT environment is enabled!) to inspect the content of ROOT file directly from the command line:
{\small
	\begin{verbatim}
		$ rootls -lr someFile.root
		
		# To check other options, check its help page:
		$ rootls --help
	\end{verbatim}
}

\noindent In the same fashion, check out ROOT command-line utilities {\rm\bf rootcp} (copy objects from ROOT files into another), and {\rm\bf rootrm} (remove objects from ROOT files directly from command line).
